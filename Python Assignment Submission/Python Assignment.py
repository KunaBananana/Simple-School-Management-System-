#---SYSTEM MAIN MENU---
def __main__(file_paths): #dictionary of file paths instead of individual variables
    # encapsulate the function (before was usign global variables)
        ...
        try:
            ...
            for line in lines:
                ...
                if len(parts) > id_field and parts[id_field] == old_username:  # Check if the username matches
                    ...
                    updated_lines.append(updated_line)  # Correctly construct the updated line
                else:
...
def viewing_student_records(file_paths, admin_username):
    ...
    print("----Viewing Student Grades----")  # More descriptive message
    if admin_username is None:
        print("You must be logged in to view student grades.")
        return
    student_username = input("Enter the student's username to check grades: ")
    print(f"\n----Grades for {student_username}----")
    try:
        with open(GRADE_FILE, "r") as file:
            ...
            for line in file:
                ...
                if len(parts) == 4:  # Validate that the line has 4 parts
                    ...
                    if student_username_from_file == student_username:  # Use the input username
                        print(f"Course: {course}, Grade: {grade}, Instructor: {instructor}")
                        grades_found = True
            if not grades_found:  # Check after the loop if no grades were found
                print(f"No grades found for student: {student_username}.")
    except FileNotFoundError:
...
#Generates a unique course ID (C01, C02, etc.).
    ...
    try:
        with open(COURSE_FILE, "r") as file:
            ...
            last_id_number = int(last_course_id[2:])  #[2:] to account for "C"
            new_id_number = last_id_number + 1
        ...
        if admin_choice == 1:
            ...
            try:
                ...
                with open(COURSE_PRICE_FILE, "a") as file:
                    file.write(f'{course_id},{course_name},{course_price}\n')
                break  # Exit the loop after successful creation
            except Exception as e:
                    ...
                    for line in lines:
                        ...
                        if len(parts) == 3:  # Check if there is 3 parts
                            ...
                            if course_id == course_id:  # identify the correct course
                                ...
                                file.write(
                                    f'{course_id},{new_course_name},{course_price}\n')
...
def update_class_schedules(file_paths, admin_username):
    ...
    while True:
        ...
        if admin_choice == 1:
            ...
            course_time = input("Enter Class Time (e.g., Monday 7AM - 9AM): ")
            course_room = input("Enter Classroom: ")
            ...
            try:
                ...
                with open(TIMETABLE_FILE, "w") as file:
                    for line in lines:
                        parts = line.strip().split(",")
                        if len(parts) == 4:  # Check if there is 4 parts
                            file.write(f'{course_id},{course_name},{course_time},{course_room}\n')
            ...
            try:  # Updating recourse
                ...
                with open(RESOURCE_FILE, "w") as file:
                    for line in lines:
                        parts = line.strip().split(",")
                        if len(parts) == 3:  # Check if there is 3 parts
                            course_id, course_name, material_file = parts
                            ...
                            # Writes the updated information back into resources.txt file
                                file.write(
                                    f'{course_id},{course_name},{material_file}\n')
...
def staff_menu(file_paths):
    ...
    def add_student():
        ...
        details = [input("Enter password: "), input("Enter contact number: "),
                   input("Enter emergency contact number: "), input("Enter address: ")]
        try:
            with open(STUDENT_FILE, "a") as file:
    ...
    def allocate_courses():
        ...
        try:
            with open(COURSE_FILE, "a") as file:
                file.write(f"{course_name},{course_id},{material_file}\n")
...
if __name__ == "__main__":
    file_paths = {  # its a dictionary object (looks up the key to find the definition (function)
        ...
        "communication_file_path": "communication.txt",
